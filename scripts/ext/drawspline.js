elation.extend("canvas.spline", new function() {
  this.getControlPoints = function(x0,y0,x1,y1,x2,y2,t){
  ...
  this.drawSpline = function(ctx, pts, t, closed){
    ctx.lineWidth=4;
    ctx.save();
    ...
    if(closed){
        ...
        for(var i=2;i<n+2;i+=2){
            var color=this.HSVtoRGB(Math.floor(240*(i-2)/(n-2)),0.8,0.8);
            if(!showDetails){color="#555555"}
            ctx.strokeStyle=this.hexToCanvasColor(color,0.75);       
            ctx.beginPath();
        ...
        var color=this.HSVtoRGB(40,0.4,0.4);  // brown
        if(!showDetails){color="#555555"}
        ctx.strokeStyle=this.hexToCanvasColor(color,0.75); 
        ctx.beginPath();
        ...
        var color=this.HSVtoRGB(240,0.8,0.8); // indigo
        if(!showDetails){color="#555555"}
        ctx.strokeStyle=this.hexToCanvasColor(color,0.75); 
        ctx.beginPath();
  ...
  this.HSVtoRGB = function(h,s,v,opacity){
  ...
  this.hexToCanvasColor = function(hexColor,opacity){
